...
<div class=" d-flex align-items-center justify-content-center p-3" style="z-index: 11">
...
<div class="signin p-0" id="otp-dialog">
    <div
        ...
        <div class="text-center gap-5 d-flex flex-column align-items-center justify-content-around">
            ...
            <p class="fs-5 mt-2 text-  text-center" id="redirectMessage-create-order-otp">
    ...
    <div class="p-1 p-md-5 main-box">
        <div class="form-header justify-content-center align-items-center d-flex flex-column">
            ...
            <p id="verification-number" class="dir-ltr card-subtitle is_number number"></p>
        ...
        <form id="userLoginForm" class="mt-4" method="POST">
            ...
            <div class="form-item" style="display: none">
                <input id="login" type="input" class="field form-control   "
                       value="{{ Session::get('phone')}}"
                       name="login"
                       autofocus>
            </div>
            <div class="pin-code-2 jpa-2  dir-ltr">
            ...
            <div class="form-item mt-5 position-relative">
                <button style="background-color:  #008451;" type="submit"
                        class="btn signin-btn otp-button">
                    @if($langCode == 'ar')
                        تحقق
                    @else
                        Verify
...
<script type="text/javascript">
    $(document).ready(function () {
        $(".pin-code-2 input").jqueryPincodeAutotab();
    })
</script>
...
<script>
    ...
    var code = '';
    var pinContainer = document.querySelector(".pin-code-2");
</script>
...
<script>
    ...
    @if($langCode == 'ar')
        toastInvalidCode = 'الكود غير صحيح'
    @else
    ...
    function removerAllField() {
        codeFields = document.querySelectorAll(".jpa-2 input")
    ...
    function getRedirectMessage(message) {
        $(".redirectMessage").removeClass("d-none");
        $(".main-box").addClass("d-none")

        const msgDate = `<span>
            @if($langCode == 'ar')
        تم انشاء طلبك بنجاح سيتصل بك احد ممثلينا بتاريخ:
        <br>
${formatDate(message)}
            @else
        Your request has been created successfully One of our representatives will contact you in:
<br>
${formatDate(message)}
            @endif
        </span>`
        ...
        const msg = `<span>
            @if($langCode == 'ar')
        تم انشاء طلبك بنجاح سيتصل بك احد ممثلينا باقرب وقت ممكن
            @else
        Your request has been created successfully One of our representatives will contact you as soon as possible

            @endif
        </span>`

        if (message !== null) {
            $("#redirectMessage-create-order-otp").html(msgDate)
        } else {
            $("#redirectMessage-create-order-otp").html(msg)
    ...
    $(document).on('click', 'button[type="submit"].otp-button', function (e) {
        ...
        if (boolField) {
            ...
            var data = Object.assign(makeOrderData, {'code': otpCode.join('')})
            ...
            $.ajax({
                ...
                dataType: "json",
                success: (response) => {
                    getRedirectMessage(response.message)
                    setTimeout(() => {
