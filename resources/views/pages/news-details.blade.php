...
@section('content')
    <main id="news-details-page">
        <section class="image-section"
                 style='background-image: url("{{$newsDetails['image']??url('/images/placeholder.png')}}")'></section>
        <div class="container py-5">
            <div class="row py-5">
                <div class="col-12"><h1>{{$newsDetails['title']}}</h1></div>
                <div class="col-12">
                    <div>{!!$newsDetails['description']!!}</div>
                </div>
            </div>
            ...
            <div class="w-100">
                ...
                <div class=" gallery-container">
                    <div class="tz-gallery">
                        <div class="row">
                            @foreach($newsDetails["images"]  as $key => $node)
                                <div class="col-12 col-md-6 col-lg-4 col-xl-3 mt-3 p-2 d-flex flex-column flex-lg-row">
                                    <a class="lightbox w-100" href="{{$node??url('/images/placeholder.png')}}">
                                        <img loading="lazy"
                                             data-src="{{$node??url('/images/placeholder.png')}}"
                                             alt="{{$newsDetails['alts'][$key] ?? ''}}" class="w-100"/>
                                    </a>
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="swiper-wrapper">
                        @foreach($allNews  as $news)
                            <div class="swiper-slide d-flex flex-column flex-lg-row">
                                <a href="/{{AppHelper::getSlug()}}/news/{{$news['slug']}}"
                                   class="text-dark text-decoration-none">
                                    <div class="row">
                                        <img loading="lazy"
                                             data-src="{{$news['image']??url('/images/placeholder.png')}}"
                                             alt="{{$news['alt'] ?? ''}}"
