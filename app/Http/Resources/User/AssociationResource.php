...
class AssociationResource extends JsonResource
{
    ...
    public function getOrders(): \Illuminate\Http\Resources\Json\AnonymousResourceCollection|array
    {
        $association = Association::whereUserId($this->id)->first();
        if ($association && $association->orders) {
            return OrderResource::collection($association->orders);
        }
